...
#!/bin/bash
    ...
    pwd
    db=/c/public
    wd=/f/00_doctor/03_data/12_XHW/05_PRJNA791216
    cd ${wd}
...
### 1.3. pipeline.sh 流程依赖数据库
    ...
    # time gunzip ${db}/usearch/*.gz # 51s
    # 查看注释数据库的格式
    sed -i 's/,;/,Unnamed;/;s/:,/:Unnamed,/g' ${db}/usearch/utax_reference_dataset_all_10.05.2021.fasta
    head -n2 ${db}/usearch/utax_reference_dataset_all_10.05.2021.fasta
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    # Cut barcode 12bp + V5 32bp in left and V7 20bp in right
    # 左端12bp Barcode+19bp上游引物为31，右端为20bp下游引物填20
...
### 4.3 基于参考去嵌合 Reference-based chimera detect
    ...
    time ${db}/win/vsearch --uchime_ref temp/otus.fa \
      -db ${db}/usearch/utax_reference_dataset_all_10.05.2021.fasta \
    ...
    # 方法2. 不去嵌合,真菌不能用上边的silva_16s_v123.fa去嵌合
...
### 5.2 物种注释-去除质体和非细菌/古菌并统计比例(可选) Remove plastid and non-Bacteria
    ...
    # utax数据库(utax_reference_dataset_all_10.05.2021.fasta)注释真菌ITS数据，UNITE数据库官方提供的数据格式有时存在错误。主要是分类级空缺的问题，我们使用sed命令对utax8.2数据库进行调整
   

    time ${db}/win/vsearch --sintax result/raw/otus.fa --db ${db}/usearch/utax_reference_dataset_all_10.05.2021.fasta \
    ...
    Rscript ${db}/script/otutab_filter_nonFungi_change.R -h
    Rscript ${db}/script/otutab_filter_nonFungi_change.R \
...
### 2.1 距离矩阵热图pheatmap
    ...
    bash ${db}/script/sp_pheatmap.sh \
      ...
      -H 'TRUE' -u 5 -v 5
    # 添加分组注释，如2，4列的基因型和地点
    cut -f 1-4 result/metadata.tsv > temp/group.txt
    # -P添加行注释文件，-Q添加列注释
    bash ${db}/script/sp_pheatmap.sh \
      -f result/beta/bray_curtis.txt \
      -H 'TRUE' -u 8 -v 6 \
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot.R \
      ...
      --legend 18 --width 110 --height 120; done
...
### 1.4 曼哈顿图
    ...
    # 上图只有6个门，切换为纲g和-L Class展示细节
    bash ${db}/script/compare_manhattan.sh -i result/compare/${compare}.txt \
       -t result/taxonomy.txt \
       -p result/tax/sum_p.txt \
       -w 183 -v 59 -s 7 -l 10 -L Phylum \
       -o result/compare/${compare}.txt
